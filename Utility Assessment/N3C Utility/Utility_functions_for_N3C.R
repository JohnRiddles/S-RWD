...
dist_btwn_cat_vars <- function(
  ...
  for (var_name in vars) {
    ...
    for (new_suffix in new_suffixes) {
      ...
      ## weighted Chi-square test to be calculated here
...
#' )
#'
dist_btwn_cont_vars <- function(orig_data, new_data, vars, wgt = NULL, ks = FALSE) {
...
cramers_v = function(orig_data, new_data, vars){
  ...
  #n_vars = length(vars)
  #out_vs_orig = matrix(NA, nrow=n_vars, ncol=n_vars)
  #rownames(out_vs_orig) = vars
  #colnames(out_vs_orig) = vars
  
  #out_vs_new = matrix(NA, nrow=n_vars, ncol=n_vars)
  #rownames(out_vs_new) = vars
  #colnames(out_vs_new) = vars
  ...
  for(i in 1:nrow(var_pairs)){
    ...
    #out_vs_orig[which(rownames(out_vs_orig)==var_pairs$VAR_1[i]), which(colnames(out_vs_orig)==var_pairs$VAR_2[i])] = V_orig
    #out_vs_orig[which(rownames(out_vs_orig)==var_pairs$VAR_2[i]), which(colnames(out_vs_orig)==var_pairs$VAR_1[i])] = V_orig
    ...
    #out_vs_new[which(rownames(out_vs_new)==var_pairs$VAR_1[i]), which(colnames(out_vs_new)==var_pairs$VAR_2[i])] = V_new
    #out_vs_new[which(rownames(out_vs_new)==var_pairs$VAR_2[i]), which(colnames(out_vs_new)==var_pairs$VAR_1[i])] = V_new
...
compare_crosstabs <- function(orig_data, new_data, vars, k = 2) {
  ...
  #out = out %>%
  #  mutate(P_ORIG = N_ORIG/sum(N_ORIG), .after=N_ORIG) %>%
  #  mutate(P_NEW = N_NEW/sum(P_NEW), .after=N_NEW)
