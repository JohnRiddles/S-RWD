...
output_path += "/Utility-Assessment/Use-Case-Data/Synthetic-Use-Case-Data/copd_cohort_procedures_of_interest"
...
@transform_df(
    ...
    procedure_occurrence=Input("ri.foundry.main.dataset.dummyid"),
...
def copd_cohort_procedures_of_interest(
    copd_cohort_de_id, concept_set_members, procedure_occurrence, customize_concept_sets
):
    ...
    procedures_df = (
        procedure_occurrence.select("person_id", "procedure_date", "procedure_concept_id")
    ...
    fusion_df = customize_concept_sets.filter(customize_concept_sets.domain.contains("procedure")).select(
