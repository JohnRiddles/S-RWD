...
output_path += "/Utility-Assessment/Use-Case-Data/copd_cohort_patient_deaths"


@transform_df(
    Output(output_path),
    death=Input("ri.foundry.main.dataset.dummyid"),
...
# Last Update - 12/6/23
# Description - This node filters the visits table for rows
# that have a discharge_to_concept_id that corresponds with the DECEASED or HOSPICE
# concept sets and combines these records with the patients in the deaths table.
# Death dates are taken from the deaths table and from the visits table
# if the patient has a discharge_to_concept_id that corresponds with the DECEASED concept set.
# Death dates are prioritized such that we filter to only plausible death dates
# from the OMOP death table and take a min prior to adding in any additional
# plausible death dates from the OMOP visits table with concept id
# belonging to the DECEASED concept set and
# take a max for patients who do not already have a death date from the OMOP death table.
# No date is retained for patients who were discharged to hospice.

def copd_cohort_patient_deaths(death, concept_set_members, copd_cohort_de_id, visit_occurrence):
    persons = copd_cohort_de_id.select("person_id", "data_extraction_date")
    death_df = death.select("person_id", "death_date").distinct().join(persons, on="person_id", how="inner")
    visits_df = (
        visit_occurrence.select("person_id", "visit_start_date", "visit_end_date", "discharge_to_concept_id")
        .distinct()
    ...
    death_from_visits_ids = list(
        concepts_df.where(F.col("concept_set_name") == "DECEASED").select("concept_id").toPandas()["concept_id"]
    )
    hospice_from_visits_ids = list(
        concepts_df.where(F.col("concept_set_name") == "HOSPICE").select("concept_id").toPandas()["concept_id"]
    )
    ...
    death_w_dates_df = (
        death_df.where((F.col("death_date") >= "2017-05-01") & (F.col("death_date") < F.col("data_extraction_date")))
        .groupby("person_id")
    ...
    death_from_visits_w_dates_df = (
        death_from_visits_df.withColumn(
            ...
            F.when(
                ...
                & (F.col("visit_end_date") < (F.col("data_extraction_date") + (365 * 2))),
    ...
    df = df.withColumn(
        "date", F.when(F.col("death_date").isNotNull(), F.col("death_date")).otherwise(F.col("visit_death_date"))
